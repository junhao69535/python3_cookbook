...
# 使用包的相对导入，使一个模块导入同一个包的另一个模块 举个例子，假设在你的
# 文件系统上有mypackage包，组织如下：
# mypackage/
#     __init__.py
#     A/
#         __init__.py
...
# 举个例子，也许有人想安装两个不同版本的软件包，只通过名称区分它们。 如果使用相对导入，
# 那一切都ok，然而使用绝对路径名很可能会出问题。
...
# 尽管使用相对导入看起来像是浏览文件系统，但是不能到定义包的目录之外。也就是说，
